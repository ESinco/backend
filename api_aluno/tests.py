...
class AlunoVisualizarPerfilTests(APITestCase):
    ...
    def test_visualizar_perfil_aluno_como_professor(self):
        ...
        self.assertEqual(self.experiencia.id, response_data['experiencias'][0]['id'])
        self.assertEqual(self.interesse.id, response_data['interesses'][0]['id'])
        self.assertEqual(response_data['avaliacao'],
                         [{'id_avaliacao': self.avaliacao.id_avaliacao,
                           'id_professor': self.professor.id,
                           'id_aluno': self.aluno.matricula,
                           'comentario': self.data_professor['avaliacao'][0]['comentario'],
                           'tags': [{'grupo': self.feedback.grupo,
                                     'id': self.feedback.id,
                                     'nome': self.feedback.nome}]}])
